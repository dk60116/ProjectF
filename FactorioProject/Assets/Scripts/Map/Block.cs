...
public partial class Block : BaseObject
{
    ...
    private const int ConveyorRunMoveMaxSegments = 32;
    ...
    private const float ConveyorDestinationBlockedRetryInterval = 1.2f;
    ...
    private readonly HashSet<BlockHandle> conveyorInsertionVisitedHandles = new HashSet<BlockHandle>();
    private readonly List<BlockHandle> conveyorTouchedBlocks = new List<BlockHandle>();
    private readonly HashSet<BlockHandle> conveyorTouchedBlockSet = new HashSet<BlockHandle>();
    private readonly Dictionary<object, bool> inputAreaCenterVisibilityRequests = new Dictionary<object, bool>();
    private readonly List<InstallationObject> belt2FBridgeCenterInstallationBuffer = new List<InstallationObject>();
    ...
    private struct ConveyorCornerMotionState
    {
        ...
        public ConveyorMotionTiming timing;
        public float progress => timing.Evaluate(Time.time);
    ...
    private ConveyorRuntimeArrays EnsureConveyorRuntimeArrays()
    {
        return conveyorRuntimeArrays ??= new ConveyorRuntimeArrays();
    }
    ...
    private readonly struct ConveyorLaneKey : IEquatable<ConveyorLaneKey>
    {
        ...
        {
            blockHandle = block != null ? block.RuntimeHandle : default;
            this.laneIndex = laneIndex;
        }

        private readonly BlockHandle blockHandle;
        private readonly int laneIndex;

        public BlockHandle BlockHandle => blockHandle;
    ...
    private readonly struct ConveyorLaneMove
    {
        ...
        {
            sourceBlockHandle = sourceBlock != null ? sourceBlock.RuntimeHandle : default;
            this.sourceLaneIndex = sourceLaneIndex;
            destinationBlockHandle = destinationBlock != null ? destinationBlock.RuntimeHandle : default;
        ...
        public readonly BlockHandle sourceBlockHandle;
        public readonly int sourceLaneIndex;
        public readonly BlockHandle destinationBlockHandle;
    ...
    private bool TryGetRuntimeConveyorBelt(out ConveyorBelt conveyorBelt)
    {
        ...
        if (mapObject is ConvayorBelt2F mappedBelt2F
            && TryGetOverlappingBelt2FBridgeCenterPassthrough(mappedBelt2F, out ConveyorBelt passthroughBelt))
        {
            conveyorBelt = passthroughBelt;
            return true;
        }
    ...
    private bool TryGetBelt2FBridgeCenterBelt(out ConvayorBelt2F belt2F)
    {
        ...
        if (mapObject is ConvayorBelt2F mappedBelt2F
            && TryGetOverlappingBelt2FBridgeCenterPassthrough(mappedBelt2F, out _))
        {
            belt2F = mappedBelt2F;
            return true;
        }
    ...
    private bool TryGetOverlappingBelt2FBridgeCenterPassthrough(
        ConvayorBelt2F belt2F,
        out ConveyorBelt passthroughBelt)
    {
        passthroughBelt = null;
        if (belt2F == null
            || !belt2F.IsBridgeCenterCoordinate(coordinate))
        {
            return false;
        ...
        belt2FBridgeCenterInstallationBuffer.Clear();
        if (!InstallationObject.CollectActiveInstallationsAtRuntimeGridCoordinate(
                coordinate,
                belt2FBridgeCenterInstallationBuffer))
        {
            return false;
        }

        for (int i = 0; i < belt2FBridgeCenterInstallationBuffer.Count; i++)
        {
            if (belt2FBridgeCenterInstallationBuffer[i] is ConveyorBelt candidate
                && !(candidate is ConvayorBelt2F)
                && IsActiveRuntimeConveyor(candidate))
            {
                passthroughBelt = candidate;
                return true;
            }
        }
    ...
    {
        insertionBlock = null;
        conveyorInsertionVisitedHandles.Clear();
        if (runtimeHandle.IsValid)
        {
            conveyorInsertionVisitedHandles.Add(runtimeHandle);
        }

        if (TryFindConnectedConveyorInsertionBlockInDirection(
                true,
                ConveyorPlacementForwardSearchDepth,
                conveyorInsertionVisitedHandles,
                out insertionBlock))
        {
            conveyorInsertionVisitedHandles.Clear();
            return true;
        ...
        bool found = TryFindConnectedConveyorInsertionBlockInDirection(
            false,
    ...
    private bool TryFindConnectedConveyorInsertionBlockInDirection(
        bool downstream,
        int maxDepth,
        HashSet<BlockHandle> visitedHandles,
        out Block insertionBlock)
    {
        ...
        for (int depth = 0; depth < maxDepth; depth++)
        {
            ...
            BlockHandle nextHandle = nextBlock != null ? nextBlock.RuntimeHandle : default;
            if (!hasNextBlock || !nextHandle.IsValid || !visitedHandles.Add(nextHandle))
            {
                return false;
            }
    ...
    private bool TryPickupSingleConveyorObjectToBag(Player player, Vector3 playerPosition, float pickupRadius, int preferredSlotIndex, int preferredItemId)
    {
        ...
        if (itemId < 0)
        {
            ClearConveyorItemForExternalRemoval(laneIndex);
    ...
    public List<int> CaptureFloorObjectState()
    {
        ...
        for (int stackIndex = 0; stackIndex < floorStacks.Count; stackIndex++)
        {
            ...
            {
                PortableObject portableObject = stack[objectIndex];
                if (ShouldPersistFloorObject(portableObject))
                {
                    floorStackItemCount++;
                }
            }
    ...
    private static bool ShouldPersistFloorObject(PortableObject portableObject)
    {
        return portableObject != null
               && (!portableObject.TryGetComponent(
                       out AnimalTemporaryDropping temporaryDropping)
                   || !temporaryDropping.IsTemporary);
    }
            ...
            if (itemId == FloorStackStateSentinel)
            {
                ...
                for (int stackIndex = 0; stackIndex < stackCount && i + 1 < itemIds.Count; stackIndex++)
                {
                    ...
                    for (int objectIndex = 0; objectIndex < stackItemCount && i + 1 < itemIds.Count; objectIndex++)
                    {
                        ...
                        // Floor drops now use one stack per cell. Fold legacy multi-stack
                        // save entries into that stack while item type and capacity allow it.
                        TryAddFloorObject(stackItemId, out _);
                    }
                }
    ...
    public void CaptureConveyorItemSaveStates(List<ConveyorItemLaneSaveState> results)
    {
        ...
        for (int laneIndex = 0; laneIndex < laneCount; laneIndex++)
        {
            ...
            else if (OwnsConveyorTransport)
            {
                ConveyorDataMotionState motion = GetTransportSaveMotion(laneIndex);
                if (motion.active) CopyDataMotionToSaveState(motion, state);
            }
    ...
    public int ClearRuntimeConveyorItemLanes(ICollection<int> laneIndices)
    {
        ...
        foreach (int laneIndex in laneIndices)
        {
            ...
            bool hadItem = HasConveyorItemAtLane(targetLaneIndex);
            PortableObject existingPortableObject = GetConveyorPortableObjectAtLane(targetLaneIndex);
            if (!hadItem && existingPortableObject == null)
            {
                continue;
            }

            ClearConveyorItemAtLane(targetLaneIndex, false);
    ...
    public int ApplyConveyorItemSaveStatesToEmptyLanes(
        IReadOnlyList<ConveyorItemLaneSaveState> states,
        ICollection<int> appliedLaneIndices)
    {
        appliedLaneIndices?.Clear();
        EnsureFloorObjectsInitialized();
        CleanupConveyorStack();
        if (!IsConveyorStackingEnabled())
        {
        ...
        if (states != null)
        {
            for (int i = 0; i < states.Count; i++)
            {
                ...
                if (HasConveyorItemAtLane(targetLaneIndex)
                    || !TrySetConveyorObjectAtLane(targetLaneIndex, state.itemId, out PortableObject portableObject))
                {
                    continue;
                }
                ...
                appliedLaneIndices?.Add(state.laneIndex);
                appliedCount++;
            }
        }
    ...
    private static void CopyCornerContinuationToSaveState(
        ...
        ConveyorItemLaneSaveState state)
    {
        if (!continuation.active || !continuation.blockHandle.IsValid)
        {
            return;
        }

        state.cornerContinuationActive = true;
        state.cornerContinuationBlockCoordinate = continuation.blockCoordinate;
        ...
        PortableObject portableObject)
    {
        ...
        if (state.useCornerMotion)
        {
            ConveyorCornerMotionState motionState = new ConveyorCornerMotionState
            {
                ...
                timing = ConveyorMotionTiming.FromPath(Time.time,
                    ResolveConveyorCornerMotionDurationPathLength(state.sourceLaneIndex,
                        state.destinationLaneIndex, state.startWorldPosition, state.pathLength, state.durationPathLength),
                    GetConveyorSpeed(), state.progress),
        ...
        if (portableObject != null)
        {
            ...
            conveyorLinearMotionStates[portableObject] = new ConveyorLinearMotionState
            {
                ...
                destinationLaneIndex = state.destinationLaneIndex,
                timing = ConveyorMotionTiming.FromPath(Time.time, state.pathLength, GetConveyorSpeed(), state.progress),
                pathLength = state.pathLength
            };
            SetConveyorPortableObjectWorldPose(portableObject, state.laneIndex, state.visualWorldPosition);
        }
    ...
    private ConveyorCornerContinuation BuildCornerContinuationFromSaveState(ConveyorItemLaneSaveState state)
    {
        ...
        continuation.blockHandle = continuationBlock.RuntimeHandle;
        continuation.blockCoordinate = continuationBlock.Coordinate;
    ...
    private void RefreshFocusMarker()
    {
        ...
        if (selectionFocusVisible)
        {
            ...
            {
                focus.SetVisible(true, focusColor);
            }
    ...
    public int ConveyorItemVisualVersion
    {
        get
        {
            if (!OwnsConveyorTransport) return conveyorItemVisualVersion;
            ConveyorTransport.Synchronize();
            return unchecked(conveyorItemVisualVersion + ConveyorTransport.Revision);
        }
    }
    ...
    public bool CanAcceptRuntimeConveyorItemAtLane(int laneIndex)
    {
        if (!IsConveyorStackingEnabled())
        {
            return false;
    ...
    private static bool IsConveyorDestinationLaneOccupied(Block destinationBlock, int destinationLaneIndex)
    {
        return destinationBlock != null
               && destinationBlock.HasConveyorItemAtLane(destinationLaneIndex);
    }

    private static bool HasConveyorDestinationBlockerForPlanFailure(Block destinationBlock, int destinationLaneIndex)
    {
        return destinationBlock != null
        ...
        float pathLength)
    {
        ...
        if (destinationBlock.OwnsConveyorTransport)
            return destinationBlock.TryAcceptTransportTransfer(this, sourceLaneIndex, destinationLaneIndex);
    ...
    public bool CompleteDueVirtualConveyorDataMotions(float now)
    {
        ...
        if (!HasConveyorDataMotionStates()
            || HasConveyorReadyLaneForMove(false, false)
    ...
    public bool NotifyStraightConveyorLineTickCompleted(out bool hasRetryWork, out bool skippedNoMoveWork)
    {
        ...
        if (!HasStraightConveyorLineRetryWorkCore())
        {
            skippedNoMoveWork = true;
            return false;
        }

        bool sleptAnyLane = SleepConveyorMoveAttempts();
        if (sleptAnyLane)
        {
            RefreshConveyorActivityRegistration(false, false);
        }

        hasRetryWork = HasStraightConveyorLineRetryWorkCore();
        return sleptAnyLane;
    }
    ...
    public bool HasStraightConveyorLineRetryWork()
    {
        ...
        CleanupConveyorStack();
        return HasStraightConveyorLineRetryWorkCore();
    }

    private bool HasStraightConveyorLineRetryWorkCore()
    {
    ...
    public void RefreshSleepAwakeDebugVisuals(bool forceVirtualRenderRefresh = false)
    {
        ...
        ConveyorRuntimeArrays runtimeArrays = EnsureConveyorRuntimeArrays();
        ...
        for (int laneIndex = 0; laneIndex < laneCount; laneIndex++)
        {
            ...
            if (laneIndex < runtimeArrays.LaneSleepAwakeDarkTintStates.Length
                && runtimeArrays.LaneSleepAwakeDarkTintStates[laneIndex] != useDarkTint)
            {
                runtimeArrays.LaneSleepAwakeDarkTintStates[laneIndex] = useDarkTint;
        ...
        for (int laneIndex = laneCount; laneIndex < runtimeArrays.LaneSleepAwakeDarkTintStates.Length; laneIndex++)
        {
            if (!runtimeArrays.LaneSleepAwakeDarkTintStates[laneIndex])
            {
                continue;
            }

            runtimeArrays.LaneSleepAwakeDarkTintStates[laneIndex] = false;
            visualStateChanged = true;
        }
    ...
    public void RefreshBeltItemLineDebugVisuals(bool forceVirtualRenderRefresh = false)
    {
        ...
        for (int laneIndex = 0; laneIndex < laneCount; laneIndex++)
        {
            ...
            if (laneIndex >= runtimeArrays.LaneBeltItemLineDebugStates.Length)
            {
                continue;
            }

            bool stateChanged = runtimeArrays.LaneBeltItemLineDebugStates[laneIndex] != useLineDebugColor;
            bool colorChanged = useLineDebugColor
                && !runtimeArrays.LaneBeltItemLineDebugColors[laneIndex].Equals(lineDebugColor);
            if (!stateChanged && !colorChanged)
            {
                continue;
            }

            runtimeArrays.LaneBeltItemLineDebugStates[laneIndex] = useLineDebugColor;
            runtimeArrays.LaneBeltItemLineDebugColors[laneIndex] = useLineDebugColor ? lineDebugColor : (Color32)Color.white;
            visualStateChanged = true;
        }

        for (int laneIndex = laneCount; laneIndex < runtimeArrays.LaneBeltItemLineDebugStates.Length; laneIndex++)
        {
            if (!runtimeArrays.LaneBeltItemLineDebugStates[laneIndex])
            {
                continue;
            }

            runtimeArrays.LaneBeltItemLineDebugStates[laneIndex] = false;
            runtimeArrays.LaneBeltItemLineDebugColors[laneIndex] = Color.white;
            visualStateChanged = true;
        }
    ...
    private void WakeConveyorMoveAttempts(bool clearBlockedSleepImmediately = false, bool queueWake = true)
    {
        ...
        for (int i = 0; runtimeArrays != null && i < runtimeArrays.NextLaneMoveAttemptTimes.Length; i++)
        {
            clearedMoveAttemptThrottle |= runtimeArrays.NextLaneMoveAttemptTimes[i] > 0f;
            runtimeArrays.NextLaneMoveAttemptTimes[i] = 0f;
        }
    ...
    public bool WakeConveyorBlockedLaneWaiter(int laneIndex)
    {
        if (!IsValidConveyorLaneIndex(laneIndex) || !HasConveyorItemAtLane(laneIndex))
        {
            return false;
        }

        bool changed = false;
        if (nextConveyorMoveAttemptTime > 0f)
        {
            nextConveyorMoveAttemptTime = 0f;
            changed = true;
        }
        ...
        ConveyorRuntimeArrays runtimeArrays = conveyorRuntimeArrays;
        if (runtimeArrays != null
            && laneIndex < runtimeArrays.NextLaneMoveAttemptTimes.Length
            && runtimeArrays.NextLaneMoveAttemptTimes[laneIndex] > 0f)
        {
            runtimeArrays.NextLaneMoveAttemptTimes[laneIndex] = 0f;
            changed = true;
        }

        changed |= ClearConveyorPlanFailureCache(laneIndex);
        ...
        TerrainGenerator activeTerrain = TerrainGenerator.Active;
        activeTerrain?.ClearConveyorBlockedLaneWaiter(this, laneIndex);
        activeTerrain?.WakeConveyorNetwork(this, false);
        RefreshConveyorActivityRegistration(false);
        activeTerrain?.QueueConveyorVacancyWake(this);
        return true;
    }

    internal void WakeConveyorVacatedLanePredecessor(int laneIndex)
    {
        // Sleep can be decided by a read-only move probe, which does not
        // register a blocked-lane waiter. Resolve the actual incoming lane too.
        if (!IsConveyorStackingEnabled()
            || !IsValidConveyorLaneIndex(laneIndex)
            || HasConveyorItemAtLane(laneIndex)
            || !TryGetConveyorPredecessor(laneIndex, out Block predecessorBlock, out int predecessorLaneIndex, out _)
            || predecessorBlock == null
            || !predecessorBlock.TryGetConveyorSuccessor(
                predecessorLaneIndex, out Block destinationBlock, out int destinationLaneIndex, out _)
            || destinationBlock != this
            || destinationLaneIndex != laneIndex)
        {
            return;
        }

        predecessorBlock.WakeConveyorBlockedLaneWaiter(predecessorLaneIndex);
    }
    ...
    public void WakeConveyorMoveAttemptsAlongRuntimeFlow()
    {
        TerrainGenerator activeTerrain = TerrainGenerator.Active;
        if (activeTerrain != null && activeTerrain.IsConveyorRuntimeRefreshDeferred)
        {
            activeTerrain.QueueDeferredConveyorMoveAttemptWakeFlow(this);
            return;
        }
    ...
    {
        NotifyTransportPortChanged();
        WakeSplitterInputs();
        WakeConveyorMoveAttemptsAlongRuntimeFlow();
        RefreshConveyorActivityRegistration();
    }

    private void ClearConveyorItemForExternalRemoval(int laneIndex)
    {
        ClearConveyorItemAtLane(laneIndex);
        RefreshConveyorActivityRegistration(false);
    }
        ...
        PortableObject requiredPortableObject)
    {
        ...
        if (floorStacks.Count == 0 && inputAreaCenterStack.Count == 0)
        {
            return false;
        }
        ...
        out float bestDistanceSqr)
    {
        ...
        Vector3 gateOriginPosition = player.transform.position;
        bool found = false;
        for (int candidateStackIndex = 0; candidateStackIndex < floorStacks.Count; candidateStackIndex++)
        {
    ...
    public bool TryGetPortableItemStackUnderRay(
        Ray ray,
        out PortableObject focusedObject,
        out float hitDistance)
    {
        ...
        List<PortableObject> focusedStack = null;
        for (int stackIndex = 0; stackIndex < floorStacks.Count; stackIndex++)
        {
            List<PortableObject> stack = floorStacks[stackIndex];
            if (!TryGetPortableItemStackRayHit(
                    stack,
                    ray,
                    out PortableObject representative,
                    out float stackHitDistance)
                || stackHitDistance >= hitDistance)
            {
                continue;
            }

            focusedObject = representative;
            focusedStack = stack;
            hitDistance = stackHitDistance;
        }

        if (!BoxObject.IsRuntimeContentBlock(this)
            && TryGetPortableItemStackRayHit(
                inputAreaCenterStack,
                ray,
                out PortableObject inputAreaRepresentative,
                out float inputAreaHitDistance)
            && inputAreaHitDistance < hitDistance)
        {
            focusedObject = inputAreaRepresentative;
            focusedStack = inputAreaCenterStack;
            hitDistance = inputAreaHitDistance;
        }

        if (focusedObject != null)
        {
            focusedObject.SetFocusStack(focusedStack);
        }

        return focusedObject != null;
    }

    private static bool TryGetPortableItemStackRayHit(
    ...
    private void EnsureFloorObjectsInitialized()
    {
        ...
        if (previousConveyorLaneCount != conveyorLaneCount) ReleaseConveyorTransport();
        ...
        bool useCornerMotion)
    {
        ...
        if (destinationBlock != null)
        {
            if (destinationBlock.TryGetConveyorSideHandoffFlow(this, sourceLaneIndex, out Vector2Int destinationFlow))
            {
                Vector3 start = GetConveyorLaneWorldPosition(sourceLaneIndex);
                Vector3 target = destinationBlock.GetConveyorLaneWorldPosition(destinationLaneIndex);
                Vector3 turn = destinationBlock.GetConveyorSideHandoffTurnPosition(
                    start, target, destinationLaneIndex, destinationFlow);
                return Vector3.Distance(start, turn) + Vector3.Distance(turn, target);
            }
    ...
    {
        if (destinationBlock != null
            && destinationBlock.TryGetConveyorSideHandoffFlow(this, sourceLaneIndex, out Vector2Int destinationFlow))
        {
            viaWorldPosition = destinationBlock.GetConveyorSideHandoffTurnPosition(
                startWorldPosition,
                destinationBlock.GetConveyorLaneWorldPosition(destinationLaneIndex),
                destinationLaneIndex,
                destinationFlow);
        ...
        float progress)
    {
        ...
        if (sourceBlock != null
            && destinationBlock != null
            && sourceBlock.TryGetConveyorLinearMoveViaWorldPosition(
                sourceLaneIndex, destinationBlock, destinationLaneIndex,
                sourceWorldPosition, out Vector3 viaWorldPosition))
        {
            return EvaluateConveyorLinearMotionWorldPosition(
                sourceWorldPosition, true, viaWorldPosition, destinationWorldPosition,
                Vector3.Distance(sourceWorldPosition, viaWorldPosition)
                    + Vector3.Distance(viaWorldPosition, destinationWorldPosition),
                progress);
        }
    ...
    private void ResetFloorObjects(bool notifyRuntime = true, bool releaseToPool = true)
    {
        ...
        if (floorObjectPool == null)
        {
            ...
            conveyorStack.Clear();
            runtimeSimulationState?.ClearConveyorLaneStorage();
    ...
    private int GetAvailableFloorCapacity(int itemId, Resource harvestedResource = null)
    {
        ...
        Transform anchor = ResolveFloorObjectDropAnchor();
        if (anchor == null || floorStacks.Count == 0)
        {
            return 0;
        }
        ...
        out List<PortableObject> stack)
    {
        ...
        stack = floorStacks.Count > 0 ? floorStacks[0] : null;
        return objectId >= 0
               && anchor != null
               && stack != null
    ...
    private bool TransferConveyorDataMotionToPortable(int laneIndex, PortableObject portableObject)
    {
        ...
        motion = EnsureConveyorDataMotionTiming(motion);
        if (motion.useCornerMotion)
        {
            conveyorCornerMotionStates[portableObject] = CreatePortableCornerMotion(motion);
            conveyorLinearMotionStates.Remove(portableObject);
        }
        else
        {
            conveyorLinearMotionStates[portableObject] = new ConveyorLinearMotionState
            {
                cornerContinuation = motion.cornerContinuation,
                startWorldPosition = motion.startWorldPosition,
                hasViaWorldPosition = motion.hasViaWorldPosition,
                viaWorldPosition = motion.viaWorldPosition,
                destinationLaneIndex = motion.destinationLaneIndex,
                pathLength = motion.pathLength,
                timing = new ConveyorMotionTiming(motion.startTime, motion.duration)
            };
            conveyorCornerMotionStates.Remove(portableObject);
        }
    ...
    private static ConveyorCornerMotionState CreatePortableCornerMotion(ConveyorDataMotionState motion)
    {
        return new ConveyorCornerMotionState
        {
            sourceLaneIndex = motion.sourceLaneIndex,
            destinationLaneIndex = motion.destinationLaneIndex,
            startWorldPosition = motion.startWorldPosition,
            pathLength = motion.pathLength,
            durationPathLength = motion.durationPathLength,
            timing = new ConveyorMotionTiming(motion.startTime, motion.duration)
        };
    }
    ...
    private int GetConveyorStoredItemIdAtLane(int laneIndex)
    {
        ...
        // Item identity is authoritative simulation data. A PortableObject in
        // conveyorStack is only an optional animation/pickup view and must never
        // resurrect an item that the data store says is absent.
        return conveyorItemIds[laneIndex];
    }
    ...
    private void SetConveyorPickupGateStateAtLane(int laneIndex, ConveyorPickupGateState gateState)
    {
        ...
        {
            item.Gate = gateState;
            ConveyorTransport.Items.SetLaneValue(TransportSlot(laneIndex), item);
    ...
    private int GetConveyorLaneOccupancyVersion(int laneIndex)
    {
        ConveyorRuntimeArrays runtimeArrays = conveyorRuntimeArrays;
        int version = runtimeArrays != null
            && laneIndex >= 0
            && laneIndex < runtimeArrays.LaneOccupancyVersions.Length
            ? runtimeArrays.LaneOccupancyVersions[laneIndex]
            : 0;
        if (OwnsConveyorTransport)
        {
            ConveyorTransport.Synchronize();
            return unchecked(version + ConveyorTransport.Revision);
        }
        return version;
    }
    ...
    private void IncrementConveyorLaneOccupancyVersion(int laneIndex)
    {
        ...
        int[] laneOccupancyVersions = EnsureConveyorRuntimeArrays().LaneOccupancyVersions;
        unchecked
        {
            laneOccupancyVersions[laneIndex]++;
            if (laneOccupancyVersions[laneIndex] == 0)
            {
                laneOccupancyVersions[laneIndex] = 1;
            }
        }
    }
    ...
    private void MoveConveyorStoredItemToLane(int sourceLaneIndex, int targetLaneIndex)
    {
        ...
        TerrainGenerator activeTerrain = TerrainGenerator.Active;
        activeTerrain?.ClearConveyorBlockedLaneWaiter(this, sourceLaneIndex);
        activeTerrain?.ClearConveyorBlockedLaneWaiter(this, targetLaneIndex);
        ClearConveyorStorageLaneRaw(sourceLaneIndex);
        IncrementConveyorLaneOccupancyVersion(targetLaneIndex);
    ...
    {
        if (OwnsConveyorTransport) ConveyorTransport.Remove(TransportSlot(laneIndex));
        if (!IsConveyorStorageLaneIndex(laneIndex))
        {
            return;
        }
        ...
        if (laneIndex == ConveyorSideExitLaneIndex)
        {
            InvalidateConveyorSuccessorCache();
        }
        ...
        ClearConveyorLaneCycleBlockedSleep(laneIndex);
        ConveyorRuntimeArrays runtimeArrays = conveyorRuntimeArrays;
        if (runtimeArrays != null && laneIndex < runtimeArrays.LaneSleepAwakeDarkTintStates.Length)
        {
            runtimeArrays.LaneSleepAwakeDarkTintStates[laneIndex] = false;
        }
    }

    private void ClearConveyorItemAtLane(int laneIndex, bool notifyLaneVacated = true)
    {
        if (!IsConveyorStorageLaneIndex(laneIndex))
        {
            return;
        ...
        activeTerrain?.ClearConveyorBlockedLaneWaiter(this, laneIndex);
        ClearConveyorStorageLaneRaw(laneIndex);
        if (notifyLaneVacated && hadItem)
        {
            activeTerrain?.NotifyConveyorLaneVacated(this, laneIndex);
        }

        MarkConveyorItemVisualDirty();
        activeTerrain?.MarkBeltItemLineDebugDirty(this);
    }
    ...
    private bool IsConveyorLaneBlockedSleep(int laneIndex)
    {
        ...
        return runtimeArrays != null
            && laneIndex >= 0
            && laneIndex < runtimeArrays.LaneBlockedSleepStates.Length
            && runtimeArrays.LaneBlockedSleepStates[laneIndex];
    }

    private bool SleepConveyorLaneBlocked(int laneIndex)
    {
        if (laneIndex < 0
            || laneIndex >= ConveyorStackLaneLimit
            || IsConveyorLaneBlockedSleep(laneIndex)
            || !HasConveyorItemAtLane(laneIndex)
        ...
        if (!HasCurrentConveyorDestinationBlockedPlanFailure(laneIndex)
        ...
        ConveyorRuntimeArrays runtimeArrays = EnsureConveyorRuntimeArrays();
        runtimeArrays.LaneBlockedSleepStates[laneIndex] = true;
        runtimeArrays.NextLaneMoveAttemptTimes[laneIndex] = 0f;
        return true;
    }
    ...
    private bool ShouldKeepSoloConveyorLaneAwake(int laneIndex)
    {
        ...
        if (IsConveyorDestinationLaneOccupied(destinationBlock, destinationLaneIndex))
        {
            return false;
        }
    ...
    private void ClearConveyorLaneBlockedSleep(int laneIndex)
    {
        ...
        if (runtimeArrays != null
            ...
            && laneIndex < runtimeArrays.LaneBlockedSleepStates.Length)
        {
            runtimeArrays.LaneBlockedSleepStates[laneIndex] = false;
        }
    }
    ...
    private bool IsConveyorLaneCycleBlockedSleep(int laneIndex)
    {
        ...
        return runtimeArrays != null
            && laneIndex >= 0
            && laneIndex < runtimeArrays.LaneCycleBlockedSleepStates.Length
            && runtimeArrays.LaneCycleBlockedSleepStates[laneIndex];
    }
    ...
    private bool SleepConveyorLaneCycleBlocked(int laneIndex)
    {
        ...
        ConveyorRuntimeArrays runtimeArrays = EnsureConveyorRuntimeArrays();
        runtimeArrays.LaneCycleBlockedSleepStates[laneIndex] = true;
        runtimeArrays.NextLaneMoveAttemptTimes[laneIndex] = 0f;
        return true;
    }
    ...
    private void ClearConveyorLaneCycleBlockedSleep(int laneIndex)
    {
        ...
        if (runtimeArrays != null
            ...
            && laneIndex < runtimeArrays.LaneCycleBlockedSleepStates.Length)
        {
            runtimeArrays.LaneCycleBlockedSleepStates[laneIndex] = false;
        }
    }
    ...
    private bool UpdateConveyorObjects(float deltaTime)
    {
        CleanupConveyorStack();
        ...
        bool hadActiveMotion = HasConveyorMotionStates();
        UpdateConveyorObjectWorldPositions(conveyorSpeed, deltaTime);
        bool movedAny = false;
        if (IsCornerConveyor())
        {
            movedAny |= TryMoveBelt2FBridgeCenterLanes();
            ...
            bool hasActiveMotion = HasConveyorMotionStates();
            UpdateConveyorBlockedRetry(movedAny);
            RefreshConveyorActivityRegistration();
            return movedAny || hadActiveMotion || hasActiveMotion;
        }
        ...
        if (!TryGetConveyorLaneLayout(out int frontLaneIndex, out int backLaneIndex))
        {
            ...
            RefreshConveyorActivityRegistration();
            return hadActiveMotion || hasActiveMotion;
        }

        movedAny |= TryMoveBelt2FBridgeCenterLanes();
        if (HasConveyorSideExitLane())
        {
            movedAny |= TryMoveConveyorLane(ConveyorSideExitLaneIndex);
        }
        movedAny |= TryMoveConveyorLane(frontLaneIndex);
        movedAny |= TryMoveConveyorLane(backLaneIndex);
        bool hasLinearMotion = HasConveyorMotionStates();
        UpdateConveyorBlockedRetry(movedAny);
        RefreshConveyorActivityRegistration();
        return movedAny || hadActiveMotion || hasLinearMotion;
    }
    ...
    private bool TryMoveBelt2FBridgeCenterLanes()
    {
        ...
        bool movedAny = false;
        const int bridgeOutputLaneIndex = 1;
        const int bridgeInputLaneIndex = 3;
        movedAny |= TryMoveConveyorLane(bridgeOutputLaneIndex);
        movedAny |= TryMoveConveyorLane(bridgeInputLaneIndex);
        return movedAny;
    }
    ...
    private bool IsConveyorLaneMoveAttemptThrottled(int laneIndex)
    {
        ...
        return runtimeArrays != null
            ...
            && laneIndex < runtimeArrays.NextLaneMoveAttemptTimes.Length
            && runtimeArrays.NextLaneMoveAttemptTimes[laneIndex] > 0f
            && Time.time < runtimeArrays.NextLaneMoveAttemptTimes[laneIndex];
    }
    ...
    private void DelayConveyorLaneMoveAttempt(int laneIndex, float delay)
    {
        ...
        float[] nextLaneMoveAttemptTimes = EnsureConveyorRuntimeArrays().NextLaneMoveAttemptTimes;
        float retryTime = Time.time + Mathf.Max(0f, delay);
        if (retryTime > nextLaneMoveAttemptTimes[laneIndex])
        {
            nextLaneMoveAttemptTimes[laneIndex] = retryTime;
            InvalidateConveyorCanMoveCaches(false);
        }
    }
    ...
    private void MarkConveyorLaneCycleBlocked(HashSet<ConveyorLaneKey> blockedLanes)
    {
        ...
        {
            if (!TryResolveRuntimeBlock(blockedLane.BlockHandle, out Block blockedBlock))
            {
                continue;
            }
        ...
        float progress)
    {
        ...
        ConveyorMotionTiming timing = ConveyorMotionTiming.FromPath(Time.time, durationPathLength, conveyorSpeed, progress);
        motionState.progress = Mathf.Clamp01(progress);
        motionState.duration = timing.Duration;
        motionState.startTime = timing.StartTime;
    ...
    private float EvaluateConveyorDataMotionProgress(ConveyorDataMotionState motionState)
    {
        ...
        return new ConveyorMotionTiming(motionState.startTime, motionState.duration).Evaluate(Time.time);
    }
    ...
    private float GetConveyorDataMotionCompletionTime(ConveyorDataMotionState motionState, float now)
    {
        ...
        return motionState.startTime + motionState.duration;
    }
    ...
    private float GetConveyorCornerContinuationRemainingPathLength(ConveyorCornerContinuation continuation)
    {
        if (!continuation.active
            || !TryResolveRuntimeBlock(continuation.blockHandle, out Block continuationBlock))
        {
            return 0f;
        }

        float pathLength = continuationBlock.ResolveConveyorCornerMotionDurationPathLength(
        ...
        Vector3 fallbackWorldPosition)
    {
        ...
        return continuationBlock.EvaluateConveyorCornerMotionWorldPosition(
            continuation.sourceLaneIndex,
            continuation.destinationLaneIndex,
            continuation.startWorldPosition,
            continuation.pathLength,
        ...
        float distance)
    {
        ...
        float durationPathLength = continuationBlock.ResolveConveyorCornerMotionDurationPathLength(
            continuation.sourceLaneIndex,
            continuation.destinationLaneIndex,
            continuation.startWorldPosition,
            continuation.pathLength,
    ...
    private void UpdateConveyorObjectWorldPositions(float conveyorSpeed, float deltaTime)
    {
        if (ShouldUseVirtualConveyorItemRendering())
        {
            UpdateVirtualConveyorObjectMotionStates();
            return;
        }
        ...
        for (int laneIndex = 0; laneIndex < laneCount; laneIndex++)
        {
            ...
            ApplyConveyorObjectRenderingMode(portableObject);
            if (UpdatePortableConveyorMotion(laneIndex, portableObject))
            {
                continue;
            }
    ...
    private void UpdateVirtualConveyorObjectMotionStates()
    {
        int laneCount = GetConveyorLaneCount();
        for (int laneIndex = 0; laneIndex < laneCount; laneIndex++)
        {
            ...
            ApplyConveyorObjectRenderingMode(portableObject);
            UpdatePortableConveyorMotion(laneIndex, portableObject);
        }
        ...
        float now)
    {
        ...
        // A move can push successors and pull predecessors in the same plan.
        // Backdating only this item by the scheduler's lateness lets it catch
        // the successor that just departed at Time.time. Start the whole plan
        // at the same time, including when arrival has no leftover delta.
        TryMoveConveyorLane(laneIndex, out _, out _, true);

        RefreshConveyorActivityRegistration(false, false);
        return true;
    }

    private bool UpdatePortableConveyorMotion(int laneIndex, PortableObject portableObject)
    {
        bool hasCornerMotion = conveyorCornerMotionStates.TryGetValue(portableObject, out ConveyorCornerMotionState cornerMotion);
        bool hasLinearMotion = conveyorLinearMotionStates.TryGetValue(portableObject, out ConveyorLinearMotionState linearMotion);
        if (!hasCornerMotion && !hasLinearMotion)
        {
            return false;
        }

        // Rendering and readiness evaluate the same per-segment clock. A new
        // segment starts at zero even if this block was asleep for a long time.
        float progress = hasCornerMotion ? cornerMotion.progress : linearMotion.progress;
        SetConveyorPortableObjectWorldPose(portableObject, laneIndex,
            GetConveyorObjectVisualWorldPosition(laneIndex, portableObject));
        if (progress < 1f - ConveyorContinuousMotionEpsilon)
        {
            return true;
        }
        ...
        MarkConveyorItemVisualDirty();
        NotifyConveyorMotionSettled();
        // The plan may move neighbours too; start every participant at now.
        TryMoveConveyorLane(laneIndex, out _, out _, true);
        return true;
    }
        ...
        bool ignoreMoveAttemptThrottle = false)
    {
        ...
        if (OwnsConveyorTransport) return false;
        if (TryMoveIntoConveyorTransport(sourceLaneIndex, out bool transportMoved, out movedDestinationBlock, out movedDestinationLaneIndex))
        ...
        out bool canMove)
    {
        ...
        if (runtimeArrays == null
            || cacheIndex < 0
            || !runtimeArrays.CanMoveCacheValid[cacheIndex]
            || runtimeArrays.CanMoveCacheFrames[cacheIndex] != Time.frameCount
            || runtimeArrays.CanMoveCacheVersions[cacheIndex] != conveyorCanMoveGlobalStateVersion)
        {
            return false;
        }

        canMove = runtimeArrays.CanMoveCacheResults[cacheIndex];
        return true;
    }
    ...
    private void CacheCanMoveConveyorLane(int sourceLaneIndex, bool ignoreMoveAttemptThrottle, bool canMove)
    {
        ...
        ConveyorRuntimeArrays runtimeArrays = EnsureConveyorRuntimeArrays();
        runtimeArrays.CanMoveCacheValid[cacheIndex] = true;
        runtimeArrays.CanMoveCacheFrames[cacheIndex] = Time.frameCount;
        runtimeArrays.CanMoveCacheVersions[cacheIndex] = conveyorCanMoveGlobalStateVersion;
        runtimeArrays.CanMoveCacheResults[cacheIndex] = canMove;
    }
    ...
    {
        return TryGetCachedConveyorPlanFailure(sourceLaneIndex, ignoreMoveAttemptThrottle, out _);
    }

    private bool TryGetCachedConveyorPlanFailure(
        int sourceLaneIndex,
        bool ignoreMoveAttemptThrottle,
        out float retryDelay)
    {
        return TryGetCachedConveyorPlanFailure(
            sourceLaneIndex,
            ignoreMoveAttemptThrottle,
            out retryDelay,
            out _);
        ...
        out bool hasDestination)
    {
        ...
        ConveyorRuntimeArrays runtimeArrays = conveyorRuntimeArrays;
        int cacheIndex = GetCanMoveConveyorLaneCacheIndex(sourceLaneIndex, ignoreMoveAttemptThrottle);
        if (runtimeArrays == null || cacheIndex < 0 || !runtimeArrays.PlanFailureCacheValid[cacheIndex])
        {
            return false;
        }

        bool expired = Time.time >= runtimeArrays.PlanFailureCacheUntilTimes[cacheIndex];
        if (runtimeArrays.PlanFailureHasDestination[cacheIndex])
        {
            bool hasResolvedDestination = TryResolveRuntimeBlock(
                runtimeArrays.PlanFailureDestinationBlockHandles[cacheIndex],
                out Block destinationBlock);
            int destinationLaneIndex = runtimeArrays.PlanFailureDestinationLaneIndices[cacheIndex];
            if (expired
                || runtimeArrays.PlanFailureSourceVersions[cacheIndex] != GetConveyorLaneOccupancyVersion(sourceLaneIndex)
                || !hasResolvedDestination
                || !destinationBlock.IsValidConveyorLaneIndex(destinationLaneIndex)
                || runtimeArrays.PlanFailureDestinationVersions[cacheIndex] != destinationBlock.GetConveyorLaneOccupancyVersion(destinationLaneIndex)
                || !HasConveyorDestinationBlockerForPlanFailure(destinationBlock, destinationLaneIndex))
            {
                TerrainGenerator.Active?.ClearConveyorBlockedLaneWaiter(this, sourceLaneIndex);
                ClearConveyorPlanFailureCacheAtIndex(cacheIndex);
                return false;
            }

            retryDelay = Mathf.Max(0f, runtimeArrays.PlanFailureCacheUntilTimes[cacheIndex] - Time.time);
            hasDestination = true;
            return true;
        }

        if (runtimeArrays.PlanFailureCacheVersions[cacheIndex] != conveyorPlanFailureGlobalStateVersion
            || expired)
        {
            ClearConveyorPlanFailureCacheAtIndex(cacheIndex);
            return false;
    ...
    private bool HasCurrentConveyorDestinationBlockedPlanFailure(int sourceLaneIndex)
    {
        return HasCurrentConveyorDestinationBlockedPlanFailure(sourceLaneIndex, false)
            || HasCurrentConveyorDestinationBlockedPlanFailure(sourceLaneIndex, true);
    }

    private bool HasCurrentConveyorDestinationBlockedPlanFailure(
        int sourceLaneIndex,
        bool ignoreMoveAttemptThrottle)
    {
        return TryGetCachedConveyorPlanFailure(
    ...
    private void CacheConveyorPlanFailure(
        int sourceLaneIndex,
        bool ignoreMoveAttemptThrottle,
        Block destinationBlock = null,
        int destinationLaneIndex = -1)
    {
        int cacheIndex = GetCanMoveConveyorLaneCacheIndex(sourceLaneIndex, ignoreMoveAttemptThrottle);
        if (cacheIndex < 0)
        {
        ...
        bool hasDestination = HasConveyorDestinationBlockerForPlanFailure(destinationBlock, destinationLaneIndex);
        float retryUntilTime = Time.time + (hasDestination
            ? ConveyorDestinationBlockedRetryInterval
            : GetConveyorBlockedRetryDelay());
        ConveyorRuntimeArrays runtimeArrays = EnsureConveyorRuntimeArrays();
        runtimeArrays.PlanFailureCacheUntilTimes[cacheIndex] =
            runtimeArrays.PlanFailureCacheValid[cacheIndex]
            && runtimeArrays.PlanFailureCacheVersions[cacheIndex] == conveyorPlanFailureGlobalStateVersion
                ? Mathf.Max(runtimeArrays.PlanFailureCacheUntilTimes[cacheIndex], retryUntilTime)
                : retryUntilTime;
        runtimeArrays.PlanFailureCacheValid[cacheIndex] = true;
        runtimeArrays.PlanFailureCacheVersions[cacheIndex] = conveyorPlanFailureGlobalStateVersion;
        runtimeArrays.PlanFailureHasDestination[cacheIndex] = hasDestination;
        runtimeArrays.PlanFailureSourceVersions[cacheIndex] = GetConveyorLaneOccupancyVersion(sourceLaneIndex);
        runtimeArrays.PlanFailureDestinationBlockHandles[cacheIndex] = hasDestination
            ? destinationBlock.RuntimeHandle
            : default;
        runtimeArrays.PlanFailureDestinationLaneIndices[cacheIndex] = hasDestination ? destinationLaneIndex : -1;
        runtimeArrays.PlanFailureDestinationVersions[cacheIndex] = hasDestination
            ? destinationBlock.GetConveyorLaneOccupancyVersion(destinationLaneIndex)
            : 0;
        TerrainGenerator activeTerrain = TerrainGenerator.Active;
        if (hasDestination)
        {
            activeTerrain?.RegisterConveyorBlockedLaneWaiter(
                this,
                sourceLaneIndex,
                destinationBlock,
                destinationLaneIndex);
    ...
    private bool ClearConveyorPlanFailureCache(int sourceLaneIndex)
    {
        ConveyorRuntimeArrays runtimeArrays = conveyorRuntimeArrays;
        if (runtimeArrays == null)
        {
        ...
        int cacheIndex = GetCanMoveConveyorLaneCacheIndex(sourceLaneIndex, false);
        if (cacheIndex >= 0 && runtimeArrays.PlanFailureCacheValid[cacheIndex])
        {
            ClearConveyorPlanFailureCacheAtIndex(cacheIndex);
            clearedAny = true;
        }

        cacheIndex = GetCanMoveConveyorLaneCacheIndex(sourceLaneIndex, true);
        if (cacheIndex >= 0 && runtimeArrays.PlanFailureCacheValid[cacheIndex])
        {
            ClearConveyorPlanFailureCacheAtIndex(cacheIndex);
            clearedAny = true;
    ...
    private void ClearConveyorPlanFailureCacheAtIndex(int cacheIndex)
    {
        ConveyorRuntimeArrays runtimeArrays = conveyorRuntimeArrays;
        if (runtimeArrays == null || cacheIndex < 0 || cacheIndex >= runtimeArrays.PlanFailureCacheValid.Length)
        {
            return;
        }

        runtimeArrays.PlanFailureCacheValid[cacheIndex] = false;
        runtimeArrays.PlanFailureCacheUntilTimes[cacheIndex] = 0f;
        runtimeArrays.PlanFailureCacheVersions[cacheIndex] = 0;
        runtimeArrays.PlanFailureSourceVersions[cacheIndex] = 0;
        runtimeArrays.PlanFailureDestinationBlockHandles[cacheIndex] = default;
        runtimeArrays.PlanFailureDestinationLaneIndices[cacheIndex] = -1;
        runtimeArrays.PlanFailureDestinationVersions[cacheIndex] = 0;
        runtimeArrays.PlanFailureHasDestination[cacheIndex] = false;
    }
        ...
        int destinationLaneIndex = -1)
    {
        if (cacheFailure)
        {
            block?.CacheConveyorPlanFailure(
                laneIndex,
                ignoreMoveAttemptThrottle,
                destinationBlock,
                destinationLaneIndex);
    ...
    private void ResolvePlannedConveyorMoveDestination(
        List<ConveyorLaneMove> plannedMoves,
        Block sourceBlock,
        int sourceLaneIndex,
        out Block destinationBlock,
        out int destinationLaneIndex)
    {
        ...
        BlockHandle sourceHandle = sourceBlock.RuntimeHandle;

        for (int i = 0; i < plannedMoves.Count; i++)
        {
            ConveyorLaneMove move = plannedMoves[i];
            if (move.sourceBlockHandle != sourceHandle || move.sourceLaneIndex != sourceLaneIndex)
            {
                continue;
            }

            TryResolveRuntimeBlock(move.destinationBlockHandle, out destinationBlock);
    ...
    private void ApplyPlannedConveyorLaneMoves(List<ConveyorLaneMove> plannedMoves)
    {
        ...
        {
            ConveyorLaneMove move = plannedMoves[i];
            if (TryResolveRuntimeBlock(move.sourceBlockHandle, out Block sourceBlock)
                && move.sourceLaneIndex >= 0
                && sourceBlock.IsValidConveyorLaneIndex(move.sourceLaneIndex)
                && sourceBlock.GetConveyorItemIdAtLane(move.sourceLaneIndex) == move.itemId
                && (move.portableObject == null
                    || sourceBlock.GetConveyorPortableObjectAtLane(move.sourceLaneIndex) == move.portableObject))
            {
                sourceBlock.ClearConveyorItemAtLane(move.sourceLaneIndex, false);
                MarkConveyorTouchedBlock(sourceBlock);
            }
        }
        ...
        for (int i = 0; i < plannedMoves.Count; i++)
        {
            ConveyorLaneMove move = plannedMoves[i];
            if (!TryResolveRuntimeBlock(move.sourceBlockHandle, out Block sourceBlock)
                || !TryResolveRuntimeBlock(move.destinationBlockHandle, out Block destinationBlock)
                || move.destinationLaneIndex < 0
                || !destinationBlock.IsValidConveyorLaneIndex(move.destinationLaneIndex)
                || move.itemId < 0)
            {
                continue;
            }
            ...
            pickupGateState.MarkSettled();
            destinationBlock.SetConveyorItemAtLane(
            ...
            if (portableObject != null)
            {
                portableObject.SetCachedParent(destinationBlock.RuntimeObjectRoot, true);
                if (!destinationBlock.ShouldUseVirtualConveyorItemRendering())
                {
                    portableObject.SetCachedActive(true);
                }

                destinationBlock.ApplyConveyorObjectRenderingMode(portableObject);
            ...
            {
                float cornerPathLength = destinationBlock.GetConveyorCornerMotionPathLength(
                    move.sourceLaneIndex,
                    move.destinationLaneIndex,
                    move.startWorldPosition);
                float cornerDurationPathLength = destinationBlock.GetSynchronizedConveyorCornerMotionDurationPathLength(
                    move.sourceLaneIndex,
                    move.destinationLaneIndex,
                    move.startWorldPosition,
                    cornerPathLength);
                if (portableObject != null)
                {
                    destinationBlock.conveyorCornerMotionStates[portableObject] = new ConveyorCornerMotionState
                    {
                        sourceLaneIndex = move.sourceLaneIndex,
                        destinationLaneIndex = move.destinationLaneIndex,
                        startWorldPosition = move.startWorldPosition,
                        timing = ConveyorMotionTiming.FromPath(Time.time, cornerDurationPathLength,
                            destinationBlock.GetConveyorSpeed()),
                        pathLength = cornerPathLength,
                        durationPathLength = cornerDurationPathLength
                    };
                    destinationBlock.conveyorLinearMotionStates.Remove(portableObject);
                    destinationBlock.conveyorItemMotionStates[move.destinationLaneIndex] = default;
                }
                else
                {
                    ConveyorDataMotionState dataMotionState = new ConveyorDataMotionState
                    ...
                    destinationBlock.conveyorItemMotionStates[move.destinationLaneIndex] =
                        destinationBlock.InitializeConveyorDataMotionTiming(dataMotionState, 0f);
                    destinationBlock.MarkConveyorItemVisualDirty();
                }
            }
            else
            {
                ...
                Vector3 linearStartWorldPosition = GetConveyorCornerContinuationEndWorldPosition(
                    cornerContinuation,
                    move.startWorldPosition);
                float pathLength = sourceBlock.GetConveyorMovePathLength(
                    move.sourceLaneIndex,
                    destinationBlock,
                    move.destinationLaneIndex,
                    false,
                ...
                bool hasViaWorldPosition = sourceBlock.TryGetConveyorLinearMoveViaWorldPosition(
                    move.sourceLaneIndex,
                    destinationBlock,
                    move.destinationLaneIndex,
                    linearStartWorldPosition,
                ...
                if (portableObject != null)
                {
                    destinationBlock.conveyorCornerMotionStates.Remove(portableObject);
                    destinationBlock.conveyorLinearMotionStates[portableObject] = new ConveyorLinearMotionState
                    {
                        ...
                        viaWorldPosition = viaWorldPosition,
                        destinationLaneIndex = move.destinationLaneIndex,
                        timing = ConveyorMotionTiming.FromPath(Time.time, pathLength, destinationBlock.GetConveyorSpeed()),
                        pathLength = pathLength
                    };
                    destinationBlock.conveyorItemMotionStates[move.destinationLaneIndex] = default;
                }
            ...
            MarkConveyorTouchedBlock(destinationBlock);
        }
        ...
        for (int i = 0; i < plannedMoves.Count; i++)
        {
            ...
            if (!TryResolveRuntimeBlock(move.sourceBlockHandle, out Block sourceBlock)
                || move.sourceLaneIndex < 0
                || !sourceBlock.IsValidConveyorLaneIndex(move.sourceLaneIndex)
                || IsConveyorLaneDestinationInPlannedMoves(plannedMoves, sourceBlock, move.sourceLaneIndex))
            {
                continue;
            }

            activeTerrain?.NotifyConveyorLaneVacated(sourceBlock, move.sourceLaneIndex);
        }
        ...
        if (activeTerrain != null)
        {
            activeTerrain.WakeAndRefreshConveyorRuntimeBlocks(
                conveyorTouchedBlocks,
                true,
                true,
                TerrainGenerator.ConveyorRuntimeWakeMode.Flow);
        }
        else
        {
            for (int i = 0; i < touchedBlockCount; i++)
            {
                if (TryResolveRuntimeBlock(conveyorTouchedBlocks[i], out Block touchedBlock))
                {
    ...
    private static bool IsConveyorLaneDestinationInPlannedMoves(
        List<ConveyorLaneMove> plannedMoves,
        Block block,
        int laneIndex)
    {
        if (plannedMoves == null || block == null)
        {
            return false;
        }

        BlockHandle blockHandle = block.RuntimeHandle;

        for (int i = 0; i < plannedMoves.Count; i++)
        {
            ConveyorLaneMove move = plannedMoves[i];
            if (move.destinationBlockHandle == blockHandle && move.destinationLaneIndex == laneIndex)
            {
                return true;
            }
        }
    ...
    {
        BlockHandle handle = block != null ? block.RuntimeHandle : default;
        if (handle.IsValid && conveyorTouchedBlockSet.Add(handle))
        {
            conveyorTouchedBlocks.Add(handle);
        }
    }
        ...
        PortableObject portableObject)
    {
        ...
        else
        {
            ...
            motionState = CreatePortableCornerMotion(dataMotionState);
        }
        ...
        continuation.active = true;
        continuation.blockHandle = runtimeHandle;
        continuation.blockCoordinate = coordinate;
    ...
    private bool CanTreatConveyorLaneAsCycleReady(ConveyorLaneKey rootLane, ConveyorLaneKey currentLane)
    {
        if (WasConveyorItemMovedThisFrame(currentLane.LaneIndex)
            || !rootLane.BlockHandle.IsValid
            || !currentLane.BlockHandle.IsValid
            || rootLane.BlockHandle != runtimeHandle
            || currentLane.BlockHandle != runtimeHandle
            || rootLane.BlockHandle != currentLane.BlockHandle
            || !IsCornerConveyor())
        {
            return false;
        }
    ...
    private bool CanReceiveConveyorHandoff(
        Block receiverBlock,
        ...
        out bool useCornerMotion)
    {
        ...
        if (destinationLaneIndex == ConveyorSideExitLaneIndex && HasConveyorSideExitLane())
        {
            sourceBlock = this;
            sourceLaneIndex = frontLaneIndex;
            return true;
        ...
        out bool useCornerMotion)
    {
        ...
        ConveyorRuntimeArrays runtimeArrays = conveyorRuntimeArrays;
        if (runtimeArrays == null || !runtimeArrays.CachedSuccessorExists[sourceLaneIndex])
        {
            return false;
        }

        if (!TryResolveRuntimeBlock(
                runtimeArrays.CachedSuccessorBlockHandles[sourceLaneIndex],
                out destinationBlock))
        {
            runtimeArrays.CachedSuccessorExists[sourceLaneIndex] = false;
            return false;
        }
        destinationLaneIndex = runtimeArrays.CachedSuccessorLaneIndices[sourceLaneIndex];
        useCornerMotion = runtimeArrays.CachedSuccessorUsesCornerMotion[sourceLaneIndex];
        return true;
    }
    ...
    private void RebuildConveyorSuccessorCache()
    {
        ...
        {
            runtimeArrays.CachedSuccessorBlockHandles[laneIndex] = default;
            runtimeArrays.CachedSuccessorLaneIndices[laneIndex] = -1;
            runtimeArrays.CachedSuccessorExists[laneIndex] = false;
            runtimeArrays.CachedSuccessorUsesCornerMotion[laneIndex] = false;
        }
        ...
        for (int laneIndex = 0; laneIndex < laneCount; laneIndex++)
        {
            ...
            runtimeArrays.CachedSuccessorBlockHandles[laneIndex] = destinationBlock.RuntimeHandle;
            runtimeArrays.CachedSuccessorLaneIndices[laneIndex] = destinationLaneIndex;
            runtimeArrays.CachedSuccessorExists[laneIndex] = true;
            runtimeArrays.CachedSuccessorUsesCornerMotion[laneIndex] = useCornerMotion;
        }
    }
    ...
    {
        if (laneIndex == ConveyorSideExitLaneIndex
            && CanUseConveyorSideExitLane()
            && TryGetConveyorFlowDirection(out Vector2Int sideExitFlow))
        {
            return ConveyorSideHandoffPath.GetApproachPosition(
                GetConveyorLaneWorldPosition(ConveyorSingleLineFrontLaneIndex),
                sideExitFlow, GetConveyorLaneHalfExtent() * 2f);
        }
    ...
    private Vector3 ResolveConveyorLaneWorldPosition(int laneIndex, Vector3 worldPosition)
    {
        if (TryGetBelt2FBridgeCenterBelt(out ConvayorBelt2F centerBelt2F))
        {
            ...
            // 교차 중심의 기본 레인(0/2)은 아래쪽 1F 벨트 소유다.
            // runtimeConveyorOverride나 커버리지 조회 결과와 무관하게 2F 높이를 적용하지 않는다.
