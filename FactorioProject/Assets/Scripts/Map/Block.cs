...
public class Block : BaseObject
{
    ...
    private readonly List<List<ConveyorSlotDotSegment>> conveyorSlotDotCachedSegmentsByLane = new List<List<ConveyorSlotDotSegment>>();
    private readonly List<float> conveyorSlotDotCachedTotalLengths = new List<float>();
    private readonly List<float> conveyorSlotDotCachedPhaseOffsets = new List<float>();
    ...
    private bool conveyorSlotDotPathCacheDirty = true;
    ...
    private static Mesh conveyorSlotDotMesh;
    ...
    private int cachedFrontColumn0LaneIndex = -1;
    ...
    public readonly struct RuntimeConveyorLaneSnapshot
    {
        public RuntimeConveyorLaneSnapshot(
            ...
            bool isSettled,
            bool isReadyToMove,
            float movementHoldRemainingSeconds,
            bool hasMotion,
            bool motionUsesCorner,
            int motionSourceLaneIndex,
            int motionDestinationLaneIndex,
            float motionProgress,
            float motionPathLength,
            float motionDurationPathLength,
            Vector3 motionStartWorldPosition,
            bool motionHasViaWorldPosition,
            Vector3 motionViaWorldPosition,
            bool hasCornerContinuation,
            Vector2Int continuationBlockCoordinate,
            int continuationSourceLaneIndex,
            int continuationDestinationLaneIndex,
            Vector3 continuationStartWorldPosition,
            float continuationStartProgress,
            float continuationPathLength,
            float continuationDurationPathLength)
        {
            BlockCoordinate = blockCoordinate;
            LaneIndex = laneIndex;
            LaneCount = laneCount;
            ItemId = itemId;
            VisualWorldPosition = visualWorldPosition;
            IsSettled = isSettled;
            IsReadyToMove = isReadyToMove;
            MovementHoldRemainingSeconds = movementHoldRemainingSeconds;
            HasMotion = hasMotion;
            MotionUsesCorner = motionUsesCorner;
            MotionSourceLaneIndex = motionSourceLaneIndex;
            MotionDestinationLaneIndex = motionDestinationLaneIndex;
            MotionProgress = motionProgress;
            MotionPathLength = motionPathLength;
            MotionDurationPathLength = motionDurationPathLength;
            MotionStartWorldPosition = motionStartWorldPosition;
            MotionHasViaWorldPosition = motionHasViaWorldPosition;
            MotionViaWorldPosition = motionViaWorldPosition;
            HasCornerContinuation = hasCornerContinuation;
            ContinuationBlockCoordinate = continuationBlockCoordinate;
            ContinuationSourceLaneIndex = continuationSourceLaneIndex;
            ContinuationDestinationLaneIndex = continuationDestinationLaneIndex;
            ContinuationStartWorldPosition = continuationStartWorldPosition;
            ContinuationStartProgress = continuationStartProgress;
            ContinuationPathLength = continuationPathLength;
            ContinuationDurationPathLength = continuationDurationPathLength;
        }

        public readonly Vector2Int BlockCoordinate;
        public readonly int LaneIndex;
        public readonly int LaneCount;
        public readonly int ItemId;
        public readonly Vector3 VisualWorldPosition;
        public readonly bool IsSettled;
        public readonly bool IsReadyToMove;
        public readonly float MovementHoldRemainingSeconds;
        public readonly bool HasMotion;
        public readonly bool MotionUsesCorner;
        public readonly int MotionSourceLaneIndex;
        public readonly int MotionDestinationLaneIndex;
        public readonly float MotionProgress;
        public readonly float MotionPathLength;
        public readonly float MotionDurationPathLength;
        public readonly Vector3 MotionStartWorldPosition;
        public readonly bool MotionHasViaWorldPosition;
        public readonly Vector3 MotionViaWorldPosition;
        public readonly bool HasCornerContinuation;
        public readonly Vector2Int ContinuationBlockCoordinate;
        public readonly int ContinuationSourceLaneIndex;
        public readonly int ContinuationDestinationLaneIndex;
        public readonly Vector3 ContinuationStartWorldPosition;
        public readonly float ContinuationStartProgress;
        public readonly float ContinuationPathLength;
        public readonly float ContinuationDurationPathLength;
    }
    ...
    {
        conveyorSlotDotPathCacheDirty = true;
    }
    ...
    private void InvalidateConveyorLaneLayoutCache()
    {
        ...
        cachedFrontColumn0LaneIndex = -1;
        cachedFrontColumn1LaneIndex = -1;
        cachedBackColumn0LaneIndex = -1;
        cachedBackColumn1LaneIndex = -1;
    ...
    public void CopyRuntimeConveyorLaneSnapshots(List<RuntimeConveyorLaneSnapshot> results)
    {
        if (results == null || !IsConveyorStackingEnabled())
        {
            return;
        }
        ...
        for (int laneIndex = 0; laneIndex < laneCount; laneIndex++)
        {
            ...
            bool hasMotion = laneIndex >= 0
                && laneIndex < conveyorItemMotionStates.Count
                && conveyorItemMotionStates[laneIndex].active;
            ConveyorDataMotionState motionState = hasMotion
                ? conveyorItemMotionStates[laneIndex]
                : default;
            ConveyorCornerContinuation continuation = hasMotion
                ? motionState.cornerContinuation
                : default;
            Vector2Int continuationCoordinate = continuation.active && continuation.block != null
                ? continuation.block.Coordinate
                : default;
            float holdRemainingSeconds = laneIndex >= 0 && laneIndex < conveyorItemMovementHoldUntilTimes.Count
                ? Mathf.Max(0f, conveyorItemMovementHoldUntilTimes[laneIndex] - Time.time)
                : 0f;

            results.Add(new RuntimeConveyorLaneSnapshot(
                Coordinate,
                laneIndex,
                laneCount,
                itemId,
                GetConveyorItemVisualWorldPosition(laneIndex),
                IsConveyorItemSettledAtLane(laneIndex),
                IsConveyorItemReadyToMoveAtLane(laneIndex),
                holdRemainingSeconds,
                hasMotion,
                hasMotion && motionState.useCornerMotion,
                hasMotion ? motionState.sourceLaneIndex : -1,
                hasMotion ? motionState.destinationLaneIndex : laneIndex,
                hasMotion ? motionState.progress : 1f,
                hasMotion ? motionState.pathLength : 0f,
                hasMotion ? motionState.durationPathLength : 0f,
                hasMotion ? motionState.startWorldPosition : GetConveyorLaneWorldPosition(laneIndex),
                hasMotion && motionState.hasViaWorldPosition,
                hasMotion ? motionState.viaWorldPosition : Vector3.zero,
                continuation.active,
                continuationCoordinate,
                continuation.active ? continuation.sourceLaneIndex : -1,
                continuation.active ? continuation.destinationLaneIndex : -1,
                continuation.active ? continuation.startWorldPosition : Vector3.zero,
                continuation.active ? continuation.startProgress : 0f,
                continuation.active ? continuation.pathLength : 0f,
                continuation.active ? continuation.durationPathLength : 0f));
        }
    }
    ...
    public bool CanUseStraightConveyorLineSimulationStructureOnly()
    {
        ...
        return Application.isPlaying
            ...
            && ShouldUseVirtualConveyorItemRendering()
            && !HasMaterializedConveyorItems()
            && TryGetConveyorLaneLayout(out _, out _, out _, out _);
    }
        ...
        out int backColumn1LaneIndex)
    {
        ...
        return IsConveyorStackingEnabled()
            ...
            && TryGetConveyorLaneLayout(
                out frontColumn0LaneIndex,
                out frontColumn1LaneIndex,
                out backColumn0LaneIndex,
                out backColumn1LaneIndex);
    }
        ...
        float pathLength)
    {
        ...
        if (hasViaWorldPosition)
        {
            ...
            if (leadInLength > ConveyorContinuousMotionEpsilon)
            {
                pathLength = Mathf.Max(0f, pathLength) + leadInLength;
            }
        }
    ...
    public bool TryGetStraightConveyorLineMotionData(
        ...
        out int backColumn0LaneIndex,
        out int backColumn1LaneIndex,
        out float withinColumn0PathLength,
        out float withinColumn1PathLength,
        out float nextColumn0PathLength,
        out float nextColumn1PathLength)
    {
        frontColumn0LaneIndex = -1;
        frontColumn1LaneIndex = -1;
        backColumn0LaneIndex = -1;
        backColumn1LaneIndex = -1;
        withinColumn0PathLength = 0f;
        withinColumn1PathLength = 0f;
        nextColumn0PathLength = 0f;
        nextColumn1PathLength = 0f;
        ...
        withinColumn0PathLength = GetConveyorPathSegmentLength(
            backColumn0LaneIndex,
            this,
            frontColumn0LaneIndex,
            false);
        withinColumn1PathLength = GetConveyorPathSegmentLength(
            backColumn1LaneIndex,
            this,
            frontColumn1LaneIndex,
            false);
        ...
        if (!nextLineBlock.TryGetStraightConveyorLineLaneIndices(
                out _,
                out _,
                out int nextBackColumn0LaneIndex,
                out int nextBackColumn1LaneIndex))
        {
            return false;
        }

        nextColumn0PathLength = GetConveyorPathSegmentLength(
            frontColumn0LaneIndex,
            nextLineBlock,
            nextBackColumn0LaneIndex,
            false);
        nextColumn1PathLength = GetConveyorPathSegmentLength(
            frontColumn1LaneIndex,
            nextLineBlock,
            nextBackColumn1LaneIndex,
            false);
        return true;
    }

    public bool TryAdvanceStraightConveyorLineBoundaryLane(int sourceLaneIndex)
    {
    ...
    public bool CanAdvanceStraightConveyorLineBoundaryLane(int sourceLaneIndex)
    {
        if (!IsConveyorStackingEnabled()
            || IsCornerConveyor()
            || !IsValidConveyorLaneIndex(sourceLaneIndex)
        ...
        return !destinationBlock.HasConveyorItemAtLane(destinationLaneIndex)
    ...
    public void WakeConveyorMoveAttemptsAround()
    {
        ...
        for (int i = 0; i < ConveyorNeighborDirections.Length; i++)
        {
            if (terrainGenerator.TryGetLoadedBlock(coordinate + ConveyorNeighborDirections[i], out Block neighborBlock)
                && neighborBlock != null)
            {
                neighborBlock.WakeConveyorMoveAttempts();
            }
        }
    }
    ...
    public void TickConveyorSlotDots(float deltaTime)
    {
        ...
        SetConveyorSlotDotsVisible(true);
    ...
    private Transform GetOrCreateConveyorSlotDot(int laneIndex)
    {
        ...
        GameObject dotObject = new GameObject($"ConveyorSlotDot_{laneIndex}");
        ...
        MeshFilter meshFilter = dotObject.AddComponent<MeshFilter>();
        meshFilter.sharedMesh = ResolveConveyorSlotDotMesh();

        MeshRenderer renderer = dotObject.AddComponent<MeshRenderer>();
        if (renderer != null)
        {
    ...
    private static Mesh ResolveConveyorSlotDotMesh()
    {
        if (conveyorSlotDotMesh != null)
        {
            return conveyorSlotDotMesh;
        }
        ...
        conveyorSlotDotMesh = new Mesh
        {
            name = "ConveyorSlotDotDisc"
        };
        conveyorSlotDotMesh.SetVertices(vertices);
        conveyorSlotDotMesh.SetTriangles(triangles, 0);
        conveyorSlotDotMesh.RecalculateBounds();
        return conveyorSlotDotMesh;
    }
    ...
    {
        EnsureConveyorSlotDotPathCache();
        if (laneIndex < 0
            || laneIndex >= conveyorSlotDotCachedSegmentsByLane.Count
            || laneIndex >= conveyorSlotDotCachedTotalLengths.Count
            || laneIndex >= conveyorSlotDotCachedPhaseOffsets.Count)
        {
            return GetConveyorLaneWorldPosition(laneIndex);
        }

        List<ConveyorSlotDotSegment> segments = conveyorSlotDotCachedSegmentsByLane[laneIndex];
        float totalLength = conveyorSlotDotCachedTotalLengths[laneIndex];
        float phaseOffset = conveyorSlotDotCachedPhaseOffsets[laneIndex];
        if (segments == null || segments.Count == 0 || totalLength <= 0.0001f)
        {
            return GetConveyorLaneWorldPosition(laneIndex);
        }
        ...
        {
            ConveyorSlotDotSegment segment = segments[i];
            if (travelDistance > segment.length && i < segments.Count - 1)
            {
    ...
    private void EnsureConveyorSlotDotPathCache()
    {
        if (!conveyorSlotDotPathCacheDirty)
        {
            return;
        }

        conveyorSlotDotPathCacheDirty = false;
        int laneCount = GetConveyorLaneCount();
        EnsureConveyorSlotDotPathCacheStorage(laneCount);
        for (int laneIndex = 0; laneIndex < laneCount; laneIndex++)
        {
            List<ConveyorSlotDotSegment> segments = conveyorSlotDotCachedSegmentsByLane[laneIndex];
            segments.Clear();
            if (TryBuildAnimatedConveyorSlotDotPath(laneIndex, segments, out float totalLength, out float phaseOffset))
            {
                conveyorSlotDotCachedTotalLengths[laneIndex] = totalLength;
                conveyorSlotDotCachedPhaseOffsets[laneIndex] = phaseOffset;
                continue;
            }

            conveyorSlotDotCachedTotalLengths[laneIndex] = 0f;
            conveyorSlotDotCachedPhaseOffsets[laneIndex] = 0f;
        }

        for (int laneIndex = laneCount; laneIndex < conveyorSlotDotCachedSegmentsByLane.Count; laneIndex++)
        {
            conveyorSlotDotCachedSegmentsByLane[laneIndex]?.Clear();
            if (laneIndex < conveyorSlotDotCachedTotalLengths.Count)
            {
                conveyorSlotDotCachedTotalLengths[laneIndex] = 0f;
            }

            if (laneIndex < conveyorSlotDotCachedPhaseOffsets.Count)
            {
                conveyorSlotDotCachedPhaseOffsets[laneIndex] = 0f;
            }
        }
    ...
    private void EnsureConveyorSlotDotPathCacheStorage(int laneCount)
    {
        while (conveyorSlotDotCachedSegmentsByLane.Count < laneCount)
        {
            conveyorSlotDotCachedSegmentsByLane.Add(new List<ConveyorSlotDotSegment>(ConveyorSlotDotPathMaxSegments));
        }

        while (conveyorSlotDotCachedTotalLengths.Count < laneCount)
        {
            conveyorSlotDotCachedTotalLengths.Add(0f);
        }

        while (conveyorSlotDotCachedPhaseOffsets.Count < laneCount)
        {
            conveyorSlotDotCachedPhaseOffsets.Add(0f);
        }
    }

    private bool TryBuildAnimatedConveyorSlotDotPath(
        int laneIndex,
        List<ConveyorSlotDotSegment> targetSegments,
        ...
        out float phaseOffset)
    {
        ...
        if (targetSegments == null || laneIndex < 0 || laneIndex >= GetConveyorLaneCount())
        {
            return false;
        }
        ...
        if (!TryAppendConveyorSlotDotSegmentsFromLaneSequence(conveyorSlotDotOrderedLaneKeys, targetSegments, ref totalLength))
        {
            return false;
        }
        ...
        if (closedLoop)
        {
            ...
            if (!TryAppendConveyorSlotDotSegment(startLaneKey, orderedStartLaneKey, targetSegments, ref totalLength))
            {
                return targetSegments.Count > 0;
            }
        ...
        for (int segmentIndex = 0; segmentIndex < ConveyorSlotDotPathMaxSegments; segmentIndex++)
        {
            ...
            if (!conveyorSlotDotVisitedLaneKeys.Add(nextLaneKey)
                || !TryAppendConveyorSlotDotSegment(currentLaneKey, nextLaneKey, targetSegments, ref totalLength))
            {
                break;
            }
    ...
    {
        if (orderedLaneKeys == null || targetSegments == null || orderedLaneKeys.Count <= 1)
        {
            return false;
        }
        ...
        {
            if (!TryAppendConveyorSlotDotSegment(orderedLaneKeys[i], orderedLaneKeys[i + 1], targetSegments, ref totalLength))
            {
                return false;
            }
        ...
        ref float totalLength)
    {
        ...
        targetSegments.Add(new ConveyorSlotDotSegment(
            sourceLaneKey.Block,
            sourceLaneKey.LaneIndex,
            destinationLaneKey.Block,
            destinationLaneKey.LaneIndex,
        ...
        Vector3 startWorldPosition)
    {
        ...
        float leadInLength = Vector3.Distance(startWorldPosition, viaWorldPosition);
        return leadInLength > ConveyorContinuousMotionEpsilon
            ? leadInLength + nominalPathLength
            : nominalPathLength;
    }
    ...
    private PortableObject MaterializeConveyorObjectForTransfer(PortableObject portableObject, int itemId, int laneIndex)
    {
        ...
        portableObject.SetWorldPosition(worldPosition);
        portableObject.CachedTransform.localRotation = Quaternion.identity;
    ...
    private void ConfigureConveyorObjectTransform(PortableObject portableObject, int laneIndex, Transform anchor)
    {
        ...
        portableObject.SetCachedParent(transform, true);
        portableObject.SetWorldPosition(GetConveyorLaneWorldPosition(laneIndex, anchor));
        portableTransform.localRotation = Quaternion.identity;
    ...
    {
        return laneIndex >= 0 && laneIndex < conveyorItemIds.Count;
    }
    ...
    private void UpdateConveyorObjects(float deltaTime)
    {
        ...
        if (IsCornerConveyor())
        {
            movedAny |= TryMoveConveyorLane(0, out _, out _, true);
            movedAny |= TryMoveConveyorLane(1, out _, out _, true);
                    ...
                    out int innerDestinationLaneIndex))
            {
                ...
                movedAny |= TryMoveConveyorLane(innerSourceLaneIndex, out _, out _, true);
            }
        ...
        movedAny |= TryMoveConveyorLane(frontColumn0LaneIndex);
        movedAny |= TryMoveConveyorLane(frontColumn1LaneIndex);
        movedAny |= TryMoveConveyorLane(backColumn0LaneIndex);
        movedAny |= TryMoveConveyorLane(backColumn1LaneIndex);
        AdvanceStartedLinearConveyorMotionStates(conveyorSpeed, deltaTime);
        UpdateConveyorBlockedRetry(movedAny);
        RefreshConveyorActivityRegistration();
    }
    ...
    private bool HasConveyorDataMotionStates()
    {
        ...
        {
            if (conveyorItemMotionStates[i].active)
            {
                return true;
            }
        }
    ...
    private bool HasMaterializedConveyorItems()
    {
        ...
        for (int laneIndex = 0; laneIndex < laneCount; laneIndex++)
        {
            if (GetConveyorPortableObjectAtLane(laneIndex) != null)
            {
                return true;
            }
        }
    ...
    private void UpdateConveyorObjectWorldPositions(float conveyorSpeed, float deltaTime)
    {
        ...
        for (int laneIndex = 0; laneIndex < laneCount; laneIndex++)
        {
            ...
            {
                portableObject.SetWorldPosition(targetPosition);
                continue;
            }

            portableObject.SetWorldPosition(Vector3.MoveTowards(
                portableObject.WorldPosition,
                targetPosition,
        ...
        int remainingCarrySteps)
    {
        ...
        {
            motionState.progress = Mathf.Clamp01(motionState.progress + (travelDistance / motionState.pathLength));
            portableObject.SetWorldPosition(EvaluateConveyorLinearMotionWorldPosition(
                motionState.cornerContinuation,
                motionState.startWorldPosition,
                motionState.hasViaWorldPosition,
                motionState.viaWorldPosition,
        ...
        int remainingCarrySteps)
    {
        ...
        if (durationPathLength <= 0.0001f || conveyorSpeed <= 0f || deltaTime <= 0f)
        {
            portableObject.SetWorldPosition(EvaluateConveyorCornerMotionWorldPosition(
                motionState.sourceLaneIndex,
                motionState.destinationLaneIndex,
                motionState.startWorldPosition,
                pathLength,
    ...
    private bool ShouldHoldConveyorLaneForPairedMove(
        int sourceLaneIndex,
        int pairedSourceLaneIndex,
        bool ignoreMoveAttemptThrottle = false)
    {
        return IsValidConveyorLaneIndex(sourceLaneIndex)
            && IsValidConveyorLaneIndex(pairedSourceLaneIndex)
            && HasConveyorItemAtLane(sourceLaneIndex)
            && HasConveyorItemAtLane(pairedSourceLaneIndex)
            && !CanMoveConveyorLane(pairedSourceLaneIndex, ignoreMoveAttemptThrottle);
    }

    private bool TryGetCornerPairedMoveSourceLane(int sourceLaneIndex, out int pairedSourceLaneIndex)
    {
        ...
        if (!TryGetConveyorSuccessor(
            ...
            || !TryGetPairedConveyorCornerTransition(
                sourceLaneIndex,
                destinationLaneIndex,
                out pairedSourceLaneIndex,
                out _))
        {
            pairedSourceLaneIndex = -1;
            return false;
    ...
    private bool CanMoveConveyorLane(int sourceLaneIndex, bool ignoreMoveAttemptThrottle = false)
    {
        ...
        bool canMove = TryPlanConveyorLaneMove(
            ...
            conveyorCanMovePlannedMoves,
            ignoreMoveAttemptThrottle,
            false)
            && conveyorCanMovePlannedMoves.Count > 0;
    ...
    private bool CanConveyorLaneJoinRun(int laneIndex, bool ignoreMoveAttemptThrottle)
    {
        ...
        return !TryGetCornerPairedMoveSourceLane(laneIndex, out int pairedSourceLaneIndex)
            || !ShouldHoldConveyorLaneForPairedMove(laneIndex, pairedSourceLaneIndex, ignoreMoveAttemptThrottle);
    }
        ...
        bool markBlockedCycles = true)
    {
        ...
        return TryAppendConveyorLaneMove(
            plannedMoves,
            currentBlock,
            currentLane.LaneIndex,
            destinationBlock,
    ...
    private void ApplyPlannedConveyorLaneMoves(List<ConveyorLaneMove> plannedMoves)
    {
        ...
        for (int i = 0; i < conveyorTouchedBlocks.Count; i++)
        {
        ...
        out int laneIndex)
    {
        ...
        if (!TryGetPreferredConveyorColumn(handoffWorldPosition, out int preferredColumn))
        {
            return false;
        }

        return TryGetConveyorLaneIndexForRowAndColumn(0, preferredColumn, out laneIndex)
    ...
    private bool TryGetBestConveyorLaneIndexFromColumn(Vector3 referenceWorldPosition, int firstCandidateLaneIndex, int secondCandidateLaneIndex, int preferredColumn, out int bestLaneIndex)
    {
        bestLaneIndex = -1;
        if (!TryGetConveyorLaneLayout(out int frontColumn0LaneIndex, out int frontColumn1LaneIndex, out int backColumn0LaneIndex, out int backColumn1LaneIndex))
        {
        ...
        float bestDistanceSqr = float.MaxValue;
        TryConsiderPreferredConveyorLane(referenceWorldPosition, firstCandidateLaneIndex, preferredFrontLaneIndex, preferredBackLaneIndex, true, ref bestLaneIndex, ref bestDistanceSqr);
        TryConsiderPreferredConveyorLane(referenceWorldPosition, secondCandidateLaneIndex, preferredFrontLaneIndex, preferredBackLaneIndex, true, ref bestLaneIndex, ref bestDistanceSqr);

        return bestLaneIndex >= 0;
    }

    private bool TryGetPreferredConveyorLaneIndexFromColumn(Vector3 referenceWorldPosition, int firstCandidateLaneIndex, int secondCandidateLaneIndex, int preferredColumn, out int bestLaneIndex)
    {
        bestLaneIndex = -1;
        if (!TryGetConveyorLaneLayout(out int frontColumn0LaneIndex, out int frontColumn1LaneIndex, out int backColumn0LaneIndex, out int backColumn1LaneIndex))
        {
        ...
        float bestDistanceSqr = float.MaxValue;
        TryConsiderPreferredConveyorLane(referenceWorldPosition, firstCandidateLaneIndex, preferredFrontLaneIndex, preferredBackLaneIndex, false, ref bestLaneIndex, ref bestDistanceSqr);
        TryConsiderPreferredConveyorLane(referenceWorldPosition, secondCandidateLaneIndex, preferredFrontLaneIndex, preferredBackLaneIndex, false, ref bestLaneIndex, ref bestDistanceSqr);

        return bestLaneIndex >= 0;
    }

    private void TryConsiderPreferredConveyorLane(
        Vector3 referenceWorldPosition,
        int laneIndex,
        int preferredFrontLaneIndex,
        int preferredBackLaneIndex,
        bool requireEmpty,
        ref int bestLaneIndex,
        ref float bestDistanceSqr)
    {
        if ((laneIndex != preferredFrontLaneIndex && laneIndex != preferredBackLaneIndex)
            || !IsValidConveyorLaneIndex(laneIndex)
            || (requireEmpty && HasConveyorItemAtLane(laneIndex)))
        {
            return;
        }
        ...
        out int backColumn1LaneIndex)
    {
        ...
        frontColumn0LaneIndex = cachedFrontColumn0LaneIndex;
        frontColumn1LaneIndex = cachedFrontColumn1LaneIndex;
        backColumn0LaneIndex = cachedBackColumn0LaneIndex;
        backColumn1LaneIndex = cachedBackColumn1LaneIndex;
    ...
    private void RebuildConveyorLaneLayoutCache()
    {
        ...
        if (IsCornerConveyor())
        {
            cachedFrontColumn0LaneIndex = 0;
            cachedFrontColumn1LaneIndex = 1;
            cachedBackColumn0LaneIndex = 2;
            cachedBackColumn1LaneIndex = 3;
            cachedConveyorLaneLayoutValid = conveyorStack.Count >= 4;
            return;
        }
        ...
        cachedFrontColumn0LaneIndex = frontLanes[0].laneIndex;
        cachedFrontColumn1LaneIndex = frontLanes[1].laneIndex;
        cachedBackColumn0LaneIndex = backLanes[0].laneIndex;
        cachedBackColumn1LaneIndex = backLanes[1].laneIndex;
        cachedConveyorLaneLayoutValid = true;
    }
    ...
    private bool TryGetBestCornerConveyorReceiveLaneIndex(Vector3 referenceWorldPosition, out int bestLaneIndex)
    {
        ...
        if (!IsValidConveyorLaneIndex(preferredLaneIndex))
        {
            return false;
        }

        if (HasConveyorItemAtLane(preferredLaneIndex))
        {
            return false;
        }
