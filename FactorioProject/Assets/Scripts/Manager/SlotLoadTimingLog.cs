...
{
    /// <summary>
    /// Keeps one slot-load measurement alive across a single-scene reload and
    /// appends the result to Tools/Log when world checkpoint restoration finishes.
    /// </summary>
    public static class SlotLoadTimingLog
    {
        private const string Header =
            "completedUtc\tslot\tstatus\tsource\ttotalMs\treadMs\tsceneWorldMs\tdetail";
        ...
        {
            CancelActive("replaced-by-new-slot-load");
            lock (Sync)
            {
                active = new Session
                {
        ...
        public static void MarkPayloadReady(int slotIndex)
        {
            lock (Sync)
            {
                if (active != null && active.SlotIndex == slotIndex && active.PayloadReadyAt == 0L)
                    active.PayloadReadyAt = SlotTimingLogSink.TimestampProvider();
            }
        }
        ...
        public static void Complete(int slotIndex, string detail = "world-ready")
        ...
        {
            double totalMilliseconds = ElapsedMilliseconds(session.StartedAt, finishedAt);
            string readMilliseconds = session.PayloadReadyAt > 0L
                ? ElapsedMilliseconds(session.StartedAt, session.PayloadReadyAt).ToString("F1", CultureInfo.InvariantCulture)
                : string.Empty;
            string sceneWorldMilliseconds = session.PayloadReadyAt > 0L
                ? ElapsedMilliseconds(session.PayloadReadyAt, finishedAt).ToString("F1", CultureInfo.InvariantCulture)
                : totalMilliseconds.ToString("F1", CultureInfo.InvariantCulture);
