...
$probeDir = Join-Path ([IO.Path]::GetTempPath()) ('ProjectF-ColliderCulling-' + [Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $probeDir | Out-Null
$files = @(
    (Join-Path $PSScriptRoot 'Checks.cs'),
    (Join-Path $repo 'FactorioProject/Assets/Scripts/Rendering/WorldColliderCullingManager.cs'))
