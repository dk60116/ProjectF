...
청크 복원이나 토폴로지 일괄 갱신으로 월드 틱만 진행되고 벨트 틱이 보류될 때도 표시 보간값이 뒤로 리셋되지 않는지 검사한다. 보류 중에는 마지막 완료 구간을 유지하고, 벨트 틱이 재개되면 같은 위치부터 전진해야 한다. 플레이어 이동 중 상호작·컬링 경로가 같은 벨트 구간을 여러 번 읽어도, 같은 점유 세대의 시각 진행률은 이전에 제출한 값보다 감소하지 않아야 한다.
