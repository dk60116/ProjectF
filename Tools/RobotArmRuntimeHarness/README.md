...
- 가득 찬 목적지의 반복 알림에도 Sleep/재시도 상태를 유지하는지, 빈칸 발견 틱에 투입을 확정한 뒤에만 애니메이션을 시작하는지 확인합니다.
- 두 팔이 한 빈칸을 동시에 계획하면 적용 순서상 첫 팔만 투입/애니메이션에 성공하고, 나머지 팔은 아이템을 보존한 채 잠드는지 확인합니다. 다음 빈칸에서 재개 및 과거 투입 전 대기 상태의 복원도 검사합니다.

시계, 전력 공급, 씬 조회, 목적지 인벤토리와 Animator 및 틱 등록 저장소는 대역입니다.
투입 계획/적용/재시도 상태 전이는 실제 소스에서 추출합니다.
