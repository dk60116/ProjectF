...
다음 틱에 중복을 합쳐 검사해. 잠든 팔은 활성 Tick 목록에서 제거하고 Wake가 다시 등록해.
활성 스냅샷을 SimulationId 순으로 복사한 뒤 전체 Plan → Apply를 유지하므로 도중에 들어온 알림은 현재 순회를 변경하지 않아.
Sleep 상태 확인이 기존 상태를 유지하면 재검사 타이머를 초기화하지 않아.
수면 중 표현은 매 Tick 순회하지 않고 시뮬레이션 경과 시간으로 필요할 때 따라잡아.
출력 경합은 양쪽이 애니메이션을 시작할 수 있지만, 실제 Apply에서 진 팔은 아이템을 유지하고 다음 빈칸을 기다려.
움직이는 화물칸은 같은 셀에서 정지하는 전환을 놓치지 않도록 재시도를 유지합니다.

## 실행
- `./Tools/RobotArmEcsHarness/Run.ps1`: 생산 코드의 알림·계획·적용·수면·수요·스케줄러와 세대 슬롯을 추출해 검사해. 5,000개 수면 대상의 Tick 순회 제외와 단일 좌표 Wake도 포함해.
